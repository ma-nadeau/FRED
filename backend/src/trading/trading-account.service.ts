import {
  ...
  export class TradingAccountService {
    constructor(private readonly tradingAccountRepository: TradingAccountRepository) {}
    ...
    async getAllTradingAccounts(): Promise<TradingAccountResponseDto[]> {
      const accounts = await this.tradingAccountRepository.getAllTradingAccounts();
  
      if (!accounts.length) {
        throw new NotFoundException('No trading accounts found.');
      }
  
      return accounts.map((account) => this.mapToTradingAccountResponseDto(account));
    }
     ...
     */
    async getTradingAccountById(accountId: number): Promise<TradingAccountResponseDto> {
      const account = await this.tradingAccountRepository.getTradingAccountById(accountId);
  ...
  async updateTradingAccount(
    ...
    const existingAccount = await this.tradingAccountRepository.getTradingAccountById(accountId);
    if (!existingAccount) {
      throw new NotFoundException('Trading account not found.');
    }

    // Update the trading account with the provided data
    const updatedAccount = await this.tradingAccountRepository.updateTradingAccount(
      accountId,
      updateTradingAccountDto,
    );
    ...
    private mapToTradingAccountResponseDto(
        ...
        return {
          ...
          type: account.type,
          tradesStock: account.tradesStock?.map((trade) => ({
            id: trade.id,
